...
class microsoft_365_email(Commands):
    def __init__(self):
        if CFG.MICROSOFT_365_CLIENT_ID and CFG.MICROSOFT_365_CLIENT_SECRET and CFG.MICROSOFT_365_REDIRECT_URI:
            self.commands = {
    ...
    def send_email(self, from_email: str, to_email: str, subject: str, content: str) -> List[str]:
        try:
            message = {
                "message": {
                    "subject": subject,
    ...
    def move_email(self, message_id: str, destination_folder_id: str) -> List[str]:
        try:
            service = build("graph.microsoft.com", "v1.0", credentials=self.credentials)
            response = service.users().messages().move(userId="me", id=message_id, body={"destinationId": destination_folder_id}).execute()
            return [f"Email moved successfully. New folder ID: {response['parentFolderId']}"]
